...
public class ClusterBlock {
    ...
    public ClusterBlock(int x, int y, int z) {
        this.x = x;
        this.y = y;
        this.z = z;
    }

    public ClusterBlock getSideBlock(int side) {
        return switch (side) {
            case FO -> getClusterFinder().getBlock(x, y, z + 1);
            case BA -> getClusterFinder().getBlock(x, y, z - 1);
            case LE -> getClusterFinder().getBlock(x - 1, y, z);
            case RI -> getClusterFinder().getBlock(x + 1, y, z);
            case TO -> getClusterFinder().getBlock(x, y + 1, z);
            case BO -> getClusterFinder().getBlock(x, y - 1, z);
            default -> null;
        };
    }

    private void assignGroup() {
        ClusterGroup firstGroup = null;

        for (int side : SIDES) {
            if ((neighbours & side) != side) continue;

            ClusterBlock neighbour = getSideBlock(side);
        ...
        if (firstGroup == null) {
            firstGroup = getClusterFinder().newGroup(state.getBlock());
        }
    ...
    public void render(Render3DEvent event) {
        ...
        ClusterBlockData blockData = getClusterFinder().getBlockData(state.getBlock());
